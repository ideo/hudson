...
module.exports = {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Contact.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Contact._attributes)
      .filter(attribute => attribute !== Contact.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Contact._attributes[attribute].type));
